...
Required JSON Schema:
{
  ...
  "tools_required": ["web_search", "os_interpreter", "project_creator", "none"],
  "pipeline_routing_tag": "fast_response" | "agentic_loop" | "research_loop" | "project_loop"
}
...
def triage_task(user_prompt):
    ...
    try:
        ...
        if any(w in user_prompt.lower() for w in ["search", "find", "news", "research"]):
...
def mock_code_generator(prompt):
    """
    Helper stub for generating code from prompt during feedback loop.
    In production, this queries the local SLM / Gemini model.
    """
    if "fail once" in prompt.lower() and "Previous attempt" not in prompt:
        return "print(undefined_variable)"  # Will fail
...
def process_prompt(user_prompt, context=[]):
    ...
    # 2. Agentic Feedback & Execution Sandbox Route
    ...
    # 3. Autonomous Project Creation Route
    ...
    # 4. Standard Fast Response Route
    else:
